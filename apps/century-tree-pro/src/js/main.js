...
if (windowGlobal) {
   ...
   */
    ...
    // The dimensions of the main element.
      ...
      const self = this;
      this.pieces = [].slice.call(
        this.el.querySelectorAll('.segmenter__piece-wrap'),
      );
   ...
   */
    ...
    // clip-path support
      ...
      const segBgEl = document.createElement('div');
      segBgEl.className = 'segmenter__background';
      segBgEl.style.backgroundImage = `url(${this.imgsrc})`;

      const segPieces = document.createElement('div');
      let segPiecesHTML = '';
      const positionsTotal = this.options.positions.length;
      ...
      for (let i = 0, len = this.options.pieces; i < len; ++i) {
        ...
        var clipRight;
        var clipBottom;
        const pos = i <= positionsTotal - 1 ? i : 0;
        const isRandom = this.options.positions === 'random';
        ...
        if (!clipPath) {
          ...
          clipRight = isRandom
            ...
            : (this.options.positions[pos].width / 100) * this.dimensions.width
            + clipLeft;
          clipBottom = isRandom
            ? (height / 100) * this.dimensions.height + clipTop
            : (this.options.positions[pos].height / 100)
        ...
        if (this.options.shadows) {
          segPiecesHTML
          += `<div class="segmenter__shadow" style="top: ${
              top
            }%; left: ${
              left
            }%; width: ${
        ...
        segPiecesHTML += clipPath
          ? `<div class="segmenter__piece" style="background-image: url(${
            this.imgsrc
    ...
    Segmenter.prototype._renderPiece = function (piece) {
      ...
      if (self.options.animation.translateZ != undefined) {
        if (typeof self.options.animation.translateZ === 'object') {
          var randArr = createRandIntOrderedArray(
            self.options.animation.translateZ.min,
            self.options.animation.translateZ.max,
            self.options.pieces,
          );
      ...
      if (self.options.animation.translateY != undefined) {
        if (typeof self.options.animation.translateY === 'object') {
          var randArr = createRandIntOrderedArray(
            self.options.animation.translateY.min,
            self.options.animation.translateY.max,
            self.options.pieces,
          );
      ...
      if (self.options.animation.translateX != undefined) {
        if (typeof self.options.animation.translateX === 'object') {
          var randArr = createRandIntOrderedArray(
            self.options.animation.translateX.min,
            self.options.animation.translateX.max,
            self.options.pieces,
          );
      ...
      if (self.options.shadows && isObjEmpty(self.options.shadowsAnimation)) {
        const shadowEl = piece.querySelector('.segmenter__shadow');
        shadowEl.style.opacity = self.options.shadowsAnimation.opacity != undefined
          ? self.options.shadowsAnimation.opacity
          : 0;
        shadowEl.style.transform = shadowEl.style.WebkitTransform = `translateX(${
          self.options.shadowsAnimation.translateX != undefined
            ? self.options.shadowsAnimation.translateX
            : 0
        }px) translateY(${
          self.options.shadowsAnimation.translateY != undefined
   ...
   */
    Segmenter.prototype.animate = function () {
      ...
      const animProps = {
        ...
        delay(el, index) {
          return (
            (self.options.pieces - index - 1) * self.options.animation.delay
          );
        },
      ...
      if (this.options.animation.translateZ != undefined) {
        const randArr = createRandIntOrderedArray(
          this.options.animation.translateZ.min,
          this.options.animation.translateZ.max,
          this.options.pieces,
        );
   ...
   */
    Segmenter.prototype._initEvents = function () {
      ...
      this.debounceResize = debounce((ev) => {
        const positionsTotal = self.options.positions.length;
        ...
        self.pieces.forEach((piece, position) => {
          let clipTop;
          let clipLeft;
          let clipRight;
          let clipBottom;
          const segmenterPiece = piece.querySelector('.segmenter__piece');

          if (self.options.positions === 'random') {
            const randT = anime.random(0, 100);
            const randL = anime.random(0, 100);
            const randW = anime.random(0, 100);
            const randH = anime.random(0, 100);
            ...
            const pos = position <= positionsTotal - 1 ? position : 0;
            ...
            clipRight = (self.options.positions[pos].width / 100) * self.dimensions.width
            + clipLeft;
            clipBottom = (self.options.positions[pos].height / 100)
              * self.dimensions.height
            + clipTop;
          }

          segmenterPiece.style.clip = `rect(${
            clipTop
          }px,${
            clipRight
          }px,${
      ...
      if (this.options.parallax) {
        const arrRand = createRandIntOrderedArray(
          this.options.parallaxMovement.min,
          this.options.parallaxMovement.max,
          this.options.pieces,
        );
        this.pieces.forEach((piece, pos) => {
          piece.setAttribute(
            'data-parallax-translation',
            typeof self.options.parallaxMovement === 'object'
              ? arrRand[pos]
              : self.options.parallaxMovement,
          );
        });
        this.mousemove = function (ev) {
          ...
          requestAnimationFrame(() => {
            self.pieces.forEach((piece) => {
              const t = piece.getAttribute('data-parallax-translation');
              const transX = (t / self.dimensions.width) * ev.clientX - t / 2;
              const transY = (t / self.dimensions.height) * ev.clientY - t / 2;
        ...
        this.handleOrientation = function () {
          ...
          requestAnimationFrame(() => {
            self.pieces.forEach((piece) => {
              const t = piece.getAttribute('data-parallax-translation');
              const transX = (t / self.dimensions.width) * y - t / 2;
              const transY = (t / self.dimensions.height) * y - t / 2;
